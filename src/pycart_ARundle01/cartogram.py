...
# Project Library
import src.pycart_ARundle01.border_util as border


def _paired_distance(X, Y):
    """Calculate paired distance between two matrices"""
    Z = X - Y
...
class Cartogram:
    ...
    def diffusion(self, max_iter=100, diff_coeff=0.25, grid_size=(100, 100)):
        ...
        def make_vgrid(s):
            ...
            rhot_2d = self.rhot.reshape(5, -1)

            # Do the Top border
            r11 = rhot_2d[s][0]
            for ix in range(1, self.xsize):
                r01 = r11
                r11 = rhot_2d[s][ix * self.ysize]

                self.vxt[s][ix][0] = -2 * (r11 - r01) / (r11 + r01)
                self.vyt[s][ix][0] = 0.0

            # Do the Bottom border
            r10 = rhot_2d[s][self.ysize-1]
            for ix in range(1, self.xsize):
                r00 = r10
                r10 = rhot_2d[s][ix * self.ysize + self.ysize-1]
            ...
            # Do the Right edge
            r01 = rhot_2d[s][(self.xsize - 1) * self.ysize]
            for iy in range(1, self.ysize):
                r00 = r01
                r01 = rhot_2d[s][(self.xsize - 1) * self.ysize + iy]
            ...
            for ix in range(1, self.xsize):
                r01 = rhot_2d[s][(ix - 1) * self.ysize]
                r11 = rhot_2d[s][ix * self.ysize]

                for iy in range(1, self.ysize):
                    r00 = r01
                    r10 = r11
                    r01 = rhot_2d[s][(ix - 1) * self.ysize + iy]
                    r11 = rhot_2d[s][ix * self.ysize + iy]
        ...
        def velocity(rx, ry, s):
           ...
           # Calculate weights for Bilinear Interpolation
            ...
            vxp = w11 * self.vxt[s][ix][iy] + w21 * self.vxt[s][ixp1][iy] + w12 * self.vxt[s][ix][iyp1] + w22 * self.vxt[s][ixp1][iyp1]
            vyp = w11 * self.vyt[s][ix][iy] + w21 * self.vyt[s][ixp1][iy] + w12 * self.vyt[s][ix][iyp1] + w22 * self.vyt[s][ixp1][iyp1]
